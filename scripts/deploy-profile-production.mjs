...
if (process.env.CONFIRM_PRODUCTION !== confirmation) {
  console.error(`Production deployment blocked. Set CONFIRM_PRODUCTION=${confirmation} after reviewing and backing up production.`);
  process.exit(1);
}

const profile = process.env.AWS_PROFILE || 'iain-hoolahan';
const work = mkdtempSync(join(tmpdir(), 'yahtzee-profile-production-'));
...
try {
  ...
  run('aws', [
    ...
    '--function-name', 'YahtzeeProfileService',
